const ChartManager = {
  ...
  renderPlotly(spec) {
    try {
      ...
      if (!spec.layout.colorway) {
        spec.layout.colorway = CONFIG.COLORES.palette;
      }

      if (spec.layout.title && typeof spec.layout.title === "string") {
        spec.layout.title = {
          text: spec.layout.title,
          font: { size: 13, color: CONFIG.COLORES.secundario, family: "Segoe UI" }
        };
      }
  ...
  // Gráficas predefinidas desde DataManager
      ...
      data: [
        {
          ...
          name: "Dilución %",
          type: "bar",
          marker: { color: CONFIG.COLORES.primario }
        },
        {
          x: datos.map(d => d.zona),
          ...
          name: "Recuperación %",
          type: "bar",
          marker: { color: CONFIG.COLORES.secundario }
        }
      ],
      layout: {
        title: "Dilución y Recuperación por Zona (ponderado por volumen)",
        barmode: "group",
        yaxis: { title: "%" }
      }
    };
  ...
  graficarEvolucionTemporal() {
    ...
    const spec = {
      data: [
        {
          x: datos.map(d => d.periodo),
          y: datos.map(d => +(d.dil * 100).toFixed(1)),
          name: "Dilución %",
          type: "scatter",
          mode: "lines+markers",
          line: { color: CONFIG.COLORES.primario, width: 2 },
          marker: { size: 6 }
        },
        {
          x: datos.map(d => d.periodo),
          ...
          type: "scatter",
          mode: "lines+markers",
          line: { color: CONFIG.COLORES.secundario, width: 2 },
          marker: { size: 6 }
        }
      ],
      layout: {
        title: "Evolución Temporal de Dilución y Recuperación",
        yaxis: { title: "%" }
      }
    };
    this.renderPlotly(spec);
  ...
  graficarBoxplotZonas() {
    ...
    this.renderPlotly({
      data: traces,
      layout: { title: "Distribución de Dilución por Zona (Boxplot)", yaxis: { title: "Dilución %" } }
    });
  },
  ...
  graficarDistribucionGauss(campo = "dil") {
    ...
    const gaussY = gaussX.map(x => {
      const norm = (1 / (std * Math.sqrt(2 * Math.PI))) * Math.exp(-0.5 * ((x - media) / std) ** 2);
      return +(norm * valores.length * step).toFixed(2);
    });
    ...
    this.renderPlotly({
      ...
      layout: {
        title: `Distribución de ${campo === "dil" ? "Dilución" : "Recuperación"} con curva gaussiana`,
        barmode: "overlay",
        xaxis: { title: "%" },
  ...
  // Procesar respuesta de Claude con gráfica
  procesarRespuesta(respuesta) {
    ...
    if (chartMatch) {
      try {
        ...
        const textoLimpio = respuesta.replace(/CHART_JSON_START[\s\S]*?CHART_JSON_END/, "").trim();
    ...
    if (plotlyMatch) {
      try {
        ...
        const textoLimpio = respuesta.replace(/PLOTLY_JSON_START[\s\S]*?PLOTLY_JSON_END/, "").trim();
        UI.addMsg(textoLimpio, "ai");
        this.renderPlotly(spec);
        return;
      } catch(e) {
